...
`comprehensive_analysis.py` benchmarks message creation speed, peak memory usage, mean operation latency, sampled consistency checks, and supported API-surface parity against the sync library. Performance rows use one warmup run plus seven measured repeats and report median/min-max values.
